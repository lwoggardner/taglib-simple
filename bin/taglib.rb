...
def process_file(file_path, path: file_path, retrieve:)
  mf = TagLib::MediaFile.read(file_path, **retrieve)
  output = mf.to_h
  output[:path] = path unless path.is_a?(IO)
  yield output.transform_values { |value| value.is_a?(Hash) ? encode_complex_property(value) : value }
end

def process_directory(directory, patterns:, retrieve:, &output)
  count = 0; now = Time.now
...
def parse_options
  ...
  parser = OptionParser.new do |opts|
    ...
    opts.on('-a', '--audio-properties [ACCURACY]', [:fast, :average, :accurate],
    ...
    opts.on('-f', '--format FORMAT', [:json, :pretty, :yaml, :pp],
